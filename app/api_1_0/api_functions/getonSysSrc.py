...
def getVsimManulInfor(country, **kwargs):
    ...
    query_str = ("SELECT "
                 ...
                 "`person_gsvc`, "
                 "`person_operator`, "
                 "(CASE WHEN `bu_group`='N' THEN 'GTBU' WHEN `bu_group`='S' THEN 'S' WHEN `bu_group`='Y' THEN 'Y' END )AS'sys', "
                 "`state`, "
                 "(CASE WHEN `slot_state`=0 THEN '在板' WHEN `slot_state`=1 THEN '脱板' WHEN `slot_state`=2 THEN '重复占位' END )AS 'slot_state', "
...
def qureyNVsim(sysDatabases, Database, country):
    """
    ...
    :param pureyStr:
    :JsonKey:
    :return: Dic Type Sql qurey results
    """
    ...
    queryStr = ("SELECT "
                ...
                "(CASE WHEN a.`occupy_status`='0' THEN '未占用' WHEN a.`occupy_status`='1' THEN '已占用' END) AS 'occupy_status', "
                "(CASE WHEN a.`slot_status`='0' THEN '未拔出' WHEN a.`slot_status`='1' THEN '已拔出' END) AS 'slot_status', "
                "(CASE WHEN a.`activate_status`='0' THEN '已激活' WHEN a.`activate_status`='1' THEN '未激活' END) AS 'activate_status', "
                "(CASE WHEN a.`identify_status`='1' THEN '未提交' WHEN a.`identify_status`='2' THEN '待认证' WHEN a.`identify_status`='3' THEN '认证通过' WHEN a.`identify_status`='4' THEN '认证失败'  END) AS 'identify_status', "
                "(CASE WHEN a.`business_status`='0' THEN '卡未停用' WHEN a.`business_status`='1' THEN '卡已停用' WHEN a.`business_status`='2' THEN '卡预停用' WHEN a.`business_status`='3' THEN '流量不足，停用' WHEN a.`business_status`='4' THEN '卡Pending' WHEN a.`business_status`='5' THEN '待测试卡' WHEN a.`business_status`='6' THEN '待下架卡' WHEN a.`business_status`='7' THEN '流量封顶停用' END) AS 'business_status', "
                "(CASE WHEN a.`bam_status`='0' THEN 'BAM正常' WHEN a.`bam_status`='1' THEN 'BAM异常' END ) AS 'bam_status', "
                "(CASE WHEN b.`package_status`='0' THEN '正常' WHEN b.`package_status`='1' THEN '限速' WHEN b.`package_status`='2' THEN '不可用' END) AS 'package_status', "
    ...
    try:
        jsonResults = getJosonData(sysStr=sysDatabases,
                                    Database=Database,
                                    query_str=queryStr)
    except KeyError as keyerr:
        errInfo = ("KeyError:{}".format(keyerr))
...
def quryonSysSrc(country):
    ...
    """
    #("查询新架构卡资源：---------------------------------------------------")
    N_VsimPackageRecord = qureyNVsim(sysDatabases='config_N',
