...
imsiInfo_sysStr = 'config_N'
imsiInfo_Database = 'glocalme_css'
...
def get_allFlower(beginLUnix, endLUnix, list_imsi):
    ...
    if list_imsi ==[]:
        ...
        connection = pymongo.MongoClient(sql_info['get140Flower']['query140Flower']['uri'])
        aggeData = list(connection.get_database(sql_info['get140Flower']['query140Flower']['db']
                                                ).get_collection(sql_info['get140Flower']['query140Flower']['collection']
                                                                ).aggregate(pipeline))
...
def get_USFlower(beginLUnix, endLUnix, list_imsi):
    ...
    if list_imsi == []:
        ...
        aggeData = list(connection.get_database(sql_info['get140Flower']['query140Flower']['db']
            ...
            sql_info['get140Flower']['query140Flower']['collection']
            ).aggregate(pipeline))
        for i in range(len(aggeData)):
            agg_id_temp = aggeData[i].pop('_id')  # {‘_id’:{}}转换成标准json数据
            aggeData[i].update(agg_id_temp)
...
def get_140countryFlower(startDate,stopDate,data):
    ...
    for imsiinfo in imsiData:
        for all in  allFlower:
            if all['imsi'] == imsiinfo['imsi']:
                imsiinfo.update({'all': all['all']})
    for imsiinfo in imsiData:
        for us in  usFlower:
            ...
            if imsiinfo['all'] !=0:
                if 'internalflower' in imsiinfo.keys():
                    imsiinfo.update({'externalflower': (imsiinfo['all'] - imsiinfo['internalflower']),
                                     'percentage': round(((imsiinfo['all'] - imsiinfo['internalflower'])/imsiinfo['all']) * 100,2)})
                else:
...
def get_140countrySrc():
    ...
    query_str = ("SELECT  "
                 ...
                 "GROUP_CONCAT(DISTINCT b.`package_type_name` ORDER BY b.`package_type_name` SEPARATOR ';') AS 'package_type', "
                 "c.`name` AS 'groupname'  "
    ...
    imsiInfo = getJosonData(sysStr=imsiInfo_sysStr,Database=imsiInfo_Database,query_str=query_str)
    return imsiInfo
...
def qury140countryFlowerStatics(begintime,endtime,TimezoneOffset):
    ...
    queryGMTOBeginTime = getGMT0StrTime(strTime=begindate,
                                        offSet=queryTimezoneOffset)
    queryGMTOEndTime = getGMT0StrTime(strTime=enddate,
                                      offSet=queryTimezoneOffset)
    Info140Country = []
    FlowerData = []
    returnData = []
